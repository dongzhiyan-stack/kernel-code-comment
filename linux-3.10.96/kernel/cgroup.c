...
/* cgroup_attach_task->find_css_set->find_existing_css_set->compare_css_sets() 进程绑定新的cgroup
   ...
   1 在for (i = 0; i < CGROUP_SUBSYS_COUNT; i++)循环那里使用cgrp->subsys[i]和oldcg->subsys[i]的cgroup_subsys_state填充
     template[CGROUP_SUBSYS_COUNT]数组。除了for循环遍历到了本次进程要绑定的struct cgroup *cgrp对应的cgroup子系统，是
...
/* 
    ...
    建立进程task_stuct与新的css_set的关系
      ...
      !!!所以css_set->subsys[]里的cgroup_subsys_state来自进程要绑定的cgroup控制结构(比如cpu cgroup子系统的struct task_group)的
      cgroup_subsys_state成员。如果一个进程没有绑定cgroup目录，那对应css_set->subsys[i]里的cgroup_subsys_state都是从父进程继承
      的默认的cgroup_subsys_state。之后进程每绑定一个cgroup目录，就要把这个cgroup目录对应的cgroup控制结构(比如cpu cgroup子系统
      的struct task_group)的成员cgroup_subsys_state按照该cgroup子系统编号保存到css_set->subsys[i]，i是cgroup子系统编号。
...
/*    
      ...
      3 一个进程只会绑定一个see_set，不管绑定多少个cgroup子系统、绑定到哪个cgroup目录，无非绑定的css_set变来变去。并且进程绑定一个
         cgroup目录后，它绑定的css_set的subsys[]数组要保存该cgroup目录对应的cgroup控制结构(比如cpu cgroup子系统的struct task_group)
         的成员cgroup_subsys_state地址，保存在subsys[]数组什么位置呢?由该cgroup目录对应的cgroup子系统编号决定。
      ...
      举例，重点来了。进程1绑定"cpu/tasks"(cgroup目录是cgroup1)和"memory/tasks"(cgroup目录是cgroup2)，绑定css_set1。
       ...
       这两个css_set都以css_set->subsys[]的cgroup_subsys_state指针为keya加入到css_set_table链表。
       ...
       template[12]={...,cgroup1对应的cgroup_subsys_state,...,cgroup2对应的cgroup_subsys_state,}，然后执行key = css_set_hash(template)，
       以template[12]里边12个cgroup子系统的cgroup_subsys_state指针为key，在css_set_table链表找到css_set1。然后执行
       compare_css_sets(css_set1, css_set2, cgroup2, template)函数，进行匹配校验。主要匹配两点，

      1循环从css_set_table上找到css_set，找到css_set1时，从css_set1和进程2之前绑定的css_set2的cg_links的链表上的取出
       cg_cgroup_link，再得到cg_cgroup_link对应的struct cgroup，比较两个struct cgroup是否相等(实际是按照cgroup子系统的编号成对比较)，
       比较11次(一共12 cgroup子系统)，有一对不相等返回false。
      2 在第一步的基础上，还有1次就是从css_set1取出的struct cgroup与进程2本次要绑定的"memory/tasks"的cgroup2属于同一个cgroup子系统，
        则要判断两个struct cgroup是否相等，不相等返回false。这些判断都通过，说明从css_set_table上找到的css_set1，就是进程绑定
        "memory/tasks"cgroup目录要绑定的css_set，css_set匹配成功。然后回到cgroup_attach_task函数，执行cgroup_task_migrate()
        把进程2的task_struct绑定到css_set1，暂时完工

       如果进程2改为绑定"memory/test2/tasks"(cgroup目录是cgroup5)，执行到cgroup_attach_task->find_css_set->find_existing_css_set()函数，
       同样执行到for (i = 0; i < CGROUP_SUBSYS_COUNT; i++)中对template[i]赋值，赋值后是
			      ...
			      unsigned long final_subsys_mask)
{
	...
	for (i = 0; i < CGROUP_SUBSYS_COUNT; i++) {
		...
		struct cgroup_subsys *ss = subsys[i];
		unsigned long bit = 1UL << i;
        //该cgroup子系统添加到系统?????对的，本次mount挂载的那个cgroup子系统的
		if (bit & added_mask) {
            ...
            //把cgroup_subsys结构移动到cgroupfs_root的subsys_list链表
			 ...
			 void *data)
{
	...
	if (root == opts.new_root) {
        ...
        //分配cpu cgroup文件系统的根目录dentry和inode
        ...
        //相当于添加本次的cpu cgroup系统到系统?????，这里处理cgroup子系统cgroup_subsys
...
/*
  ...
  1 分配骗人的struct task_and_cgroup *tc结构，执行下边的do...while_each_thread(leader, tsk)把进程及其线程的task_struct、
    struct old css_set、struct old cgroup信息保存到struct task_and_cgroup。该函数经常从struct task_and_cgroup取出这些信息。
			      ...
			      bool threadgroup)//cgroup_attach_task详细过程还是看find_existing_css_set()函数
{
	...
	if (threadgroup)
		...
		group_size = 1;
    //分配struct flex_array，顺带着分配内存，下边执行flex_array_put()向group指向的struct flex_array填充数据
	...
	for (i = 0; i < group_size; i++) {
        ...
        //tc->cgrp是该进程之前绑定的struct cgroup，tc->task是本次要绑定的进程task结构,tc->cg是本次绑定操作新分配的struct css_set
		tc = flex_array_get(group, i);
						...
						size_t nbytes, loff_t *ppos)
{
    ...
    //通过父目录的dentry获取父目录对应的struct cgroup结构体，这个struct cgroup就代表了这个目录
	struct cgroup *cgrp = __d_cgrp(file->f_dentry->d_parent);
    //该cgroup目录不存在
	if (cgroup_is_removed(cgrp))
		return -ENODEV;
    //根据情况调用struct cftype *cft的的写函数.echo设置进程运行时间要调用的write函数是 cpu_cfs_quota_write_s64()
...
{
    //cgroup子系统父目录的dentry，下边就是在这个dentry下创建cgroup子文件"tasks"等等
    ...
    //这里根据cgroup子系统，拼接它的cgroup文件的名字，比如"cpu.cfs_quota_us"
	...
	if (!cfe)
		return -ENOMEM;
    //在父目录base下，查找或者创建name指定的文件dentry
	dentry = lookup_one_len(name, dir, strlen(name));
	if (IS_ERR(dentry)) {
		error = PTR_ERR(dentry);
		goto out;
	}
    //cfe保存这个cgroup文件的基本信息
	cfe->type = (void *)cft;
	cfe->dentry = dentry;
	dentry->d_fsdata = cfe;
	...
	if (!error) {
        //把新分配的cfe添加到cgroup父目录的struct cgroup的files链表
 ...
 */
//把cgroup子系统特有的cgroup控制文件cftype数组添加到cgorup子系统struct cgroup_subsys的cftsets链表.blkcg_policy_register->cgroup_add_cftypes
...
static struct cftype files[] = {
	{
		...
		.write_u64 = cgroup_tasks_write,//绑定每个进程到cgroup子系统
		.release = cgroup_pidlist_release,
			       ...
			       unsigned long subsys_mask)
{
	int err;
	struct cgroup_subsys *ss;
    //从struct cftype files[]结构体获取cgroup基本的文件cftype，比如"task"、"release_agent"等基本cgroup文件，
   ...
   //cgrp->root指向根cgroupfs_root，这是遍历根cgroupfs_root的subsys_list链表上的cpu、memory、blkio等cgroup_subsys,再通过cgroup_subsys
   //的成员sibling container_of找到cftype_set，再找到cftype_set的cfts指向的cftype，这个cftype是每个cgroup_subsys特有的cftype。上边的是
   //创建cgroup基本文件，这里的cftype是cpu、memory、blkio特有的cgroup文件。
	for_each_subsys(cgrp->root, ss) {
		struct cftype_set *set;
        //这是限定只有与subsys_mask指定的cgroup子系统匹配才能执行下边的cgroup_addrm_files()创建特定cgroup子系统的cgroup文件。
        //比如在cpu cgroup目录下创建cgroup文件，该if只有是ss是cpu的cgroup_subsys才不成立
		if (!test_bit(ss->subsys_id, &subsys_mask))
			continue;
        
        //遍历cgroup_subsys该cgroup子系统cftsets链表上的cftype_set，再通过cftype_set的cfts找到cgroup子系统特有的struct cftype[]，创建
        //cftype指定的该cgroup子系统特有的cgroup文件。cgroup_subsys子系统特有的struct cftype[]有哪些呢?如struct cftype cpu_files[]，
        //struct cftype blkcg_files[]，struct cftype throtl_files[].更详细见struct cftype结构体描述。
		list_for_each_entry(set, &ss->cftsets, node)
			       ...
			       struct cgroup *cgrp)
{
    ...
    //cgroup结构的subsys[]成员赋值本次croup对应的css结构，数组位置是subsys_id
			     ...
			     umode_t mode)
{
    ...
    //看着像是cpu cgroup下每创建一个目录，都会创建一个task_group呀，其他cgroup系统估计也是这样
        ...
        //初始化struct cgroup_subsys_state css，赋值cgrp->subsys[ss->subsys_id]=css
...
static void __init_or_module cgroup_init_cftsets(struct cgroup_subsys *ss)
{
	 ...
	 */
	if (ss->base_cftypes) {
        //cgroup_subsys的cftype_set的cfts指向cgroup子系统cgroup_subsys的成员base_cftypes指向的struct cftype数组
		ss->base_cftset.cfts = ss->base_cftypes;
        //把cgroup子系统struct cgroup_subsys的成员base_cftypes指向的struct cftype数组添加到cgroup_subsys的cftsets链表
