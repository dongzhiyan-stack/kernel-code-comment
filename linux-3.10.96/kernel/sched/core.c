...
{
	int cpu = smp_processor_id();
    //从per cpu变量runqueues变量获取当前cpu运行队列
	struct rq *rq = cpu_rq(cpu);
    //curr代表当前运行进程的task结构
...
pick_next_task(struct rq *rq)
{
	...
	for_each_class(class) {
		p = class->pick_next_task(rq);//pick_next_task_fair
		if (p)
			return p;
	}
...
SYSCALL_DEFINE0(sched_yield)
{
	...
	current->sched_class->yield_task(rq);//yield_task_fair
	...
	sched_preempt_enable_no_resched();//不让抢占???
...
again:
    //当前进程所在cpu的队列，与本地cpu运行队列可能不一样
	p_rq = task_rq(p);
	/*
	 * If we're the only runnable task on the rq and target rq also
	 * has only one task, there's absolutely no point in yielding.
	 */
	//如果本地cpu运行队列和进程所在的cpu运行队列只有一个运行的进程，直接返回，不再放弃
	...
	while (task_rq(p) != p_rq) {//这个不成立吧，肯定相等，因为上边才 p_rq = task_rq(p);
...
static struct cgroup_subsys_state *cpu_cgroup_css_alloc(struct cgroup *cgrp)
{
    ...
    //由本次cgroup结构体得到父task_group结构
	parent = cgroup_tg(cgrp->parent);
    //分配新的task_group并为它的成员赋值,里边用到parent->se[i]，即对应cpu上的父调度组的se
    //与本次分配的se构成关系
