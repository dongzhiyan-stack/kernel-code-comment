...
//每个进程组有自己独立的运行队列cfs_rq，比如有两个调度实体A和B，B是进程组，pick_next时
//先找到A和B的父调度队列cfs_rq，然后选择left_most调度队列最左边的调度实体B，而B是进程组
//则从调度实体的my_q取出B进程组的运行队列cfs_rq，再从cfs_rq取出最优调度实体，该调度实体是
//进程就返回，下次就调度该进程，是进程组再接着上边的循环。
struct cfs_rq {
...
#ifdef CONFIG_CFS_BANDWIDTH//cfs带宽控制
	int runtime_enabled;
	u64 runtime_expires;
...
struct rq {
	...
	/* cpu of this runqueue: *//* 该运行队列所属CPU */  
	int cpu;
	int online;
...
struct sched_class {
    ...
    //调度实体入队，及放入红黑树，并对实体nr_running加1
	void (*enqueue_task) (struct rq *rq, struct task_struct *p, int flags);
    //调度实体出队，从红黑树中删除，并对实体nr_running减1
	void (*dequeue_task) (struct rq *rq, struct task_struct *p, int flags);
    /* 放弃CPU，在 compat_yield sysctl 关闭的情况下，该函数实际上执行先出队后入队；
     在这种情况下，它将调度实体放在红黑树的最右端 */  
    ...
    //将当前运行的进程放回运行队列，接着执行pick_next_task
