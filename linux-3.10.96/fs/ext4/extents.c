			  ...
			  struct ext4_extent *newext, int at)
{
    ...
    //neh指向新分配的叶子节点首内存的头结构ext4_extent_header
    ...
    //i初值是ext4_extent B+树最后一层索引节点层数
	i = depth - 1;
    //循环k次保证把at那一层的ext4_extent B+树索引节点到最后一层索引节点中，每一层path[i].p_idx指向的ext4_extent_idx结构到
    //最后一个ext4_extent_idx结构之间的ext4_extent_idx结构，都复制到ablocks[--a]即newblock这个物理块映射bh。注意，是从ext4_extent B+
    //树最下层的索引节点向上开始复制，因为i的初值是depth - 1，这是ext4_extent B+树最下边一层索引节点的层数
	while (k--) {
		oldblock = newblock;
        //新取出一个物理块对应的块号newblock，这个物理块保存索引节点ext4_extent_header头结构+N个ext4_extent_idx结构的4K数据
        //或者保存叶子节点ext4_extent_header头结构+N个ext4_extent结构的4K数据
		newblock = ablocks[--a];
        //newblock物理块映射的bh
		bh = sb_getblk(inode->i_sb, newblock);
		if (unlikely(!bh)) {
...
repeat:
	...
	//ext4 extent B+树索引节点或者叶子节点 有 空闲条目entry,此时的i表示ext4 extent B+树有空闲entry的那一层索引节点或叶子结点层数。
	...
	/*到这个分支，ext4 extent B+树索引节点的ext4_extent_idx和叶子节点的ext4_extent个数全爆满，全都没有空闲条目entry，
	  就是说ext4 extent B+树全爆满了，只能增加执行ext4_ext_grow_indepth()增加ext4 extent B+树深度了。理解这点非常关键*/
		 ...
		 */
		depth = ext_depth(inode);
        //path[depth].p_hdr指向的叶子结点保存ext4_extent结构达到eh_max，即ext4_exteng结构爆满，则goto repeat寻找有空闲entry的索引节点，
...
has_space:
	...
	if (!nearex) {//ext4 extent B+树叶子节点没有一个extent结构，可能吗?可能的
        ...
        //这是计算nearex这个ext4_extent结构到叶子节点最后一个ext4_extent结构之间的ext4_extent结构个数
		...
		if (len > 0) {
            ...
            //nearex这个ext4_extent结构的空间，下边是用newext来填充，这样终于把newex插入ext4_extent B+树了
