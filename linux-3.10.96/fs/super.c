	...
	int (*fill_super)(struct super_block *, void *, int))
{
	...
	if (!(flags & MS_RDONLY))
		mode |= FMODE_WRITE;
    //查找到/dev/mmcblk0p5块设备的dentry、mnt、inode结构，由inode结构得到bdev，执行著名
    //的__blkdev_get函数然后open这个块设备
    ...
    //创建superblock结构并初始化其成员
	s = sget(fs_type, test_bdev_super, set_bdev_super, flags | MS_NOSEC,
		 bdev);
	mutex_unlock(&bdev->bd_fsfreeze_mutex);
	if (IS_ERR(s))
		goto error_s;
    //ext4文件系统的root dentry已经有了
	if (s->s_root) {
     ...
     /*
          读取ext4文件系统flash中的超级快数据，并分析超级快数据，给
       ext4_super_block、ext4_sb_info、super_block赋值。得到ext4文件系统的root inode，
      以"/"目录名字分配并初始化root dentry，并对vfsmont初始化
      */
        //ext4_fill_super
