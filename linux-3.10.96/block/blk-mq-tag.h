...
struct blk_mq_tags {
	...
	//剩余的30个分配?????这些req的下标是70+0/1/2/3等等。注意，代码里static_rqs[]数组下标用tag变量表示，即static_rqs[tag]
    ...
    //在blk_mq_get_driver_tag()，hctx->tags->rqs[req->tag]=req，req来自进程的plug->mq_list链表，赋值后就建立了req与硬件队列的关系
