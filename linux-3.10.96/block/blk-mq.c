...
struct request *__blk_mq_alloc_request(struct blk_mq_alloc_data *data, int rw)
{
   ...
   //static_rqs[]数组的下标
	...
	if (tag != BLK_MQ_TAG_FAIL) {
        ...
        //看到没，这里才是从tags->static_rqs[tag]得到空闲的req
		rq = tags->static_rqs[tag];

		if (data->flags & BLK_MQ_REQ_INTERNAL) {//调度算法
			rq->tag = -1;
...
//依次取出req->bio链表上所有req对应的bio,一个一个更新bio结构体成员数据，执行bio的回调函数.还更新eq->__data_len和req->buffer。
...
//rq即req来自当前进程的plug->mq_list链表，现在赋值到了硬件队列hctx->tags->rqs[rq->tag]
//结构，就建立了req与硬件队列的关系吧
bool blk_mq_get_driver_tag(struct request *rq, struct blk_mq_hw_ctx **hctx,
			   bool wait)//req来自当前进程plug->mq_list链表
{
...
void blk_mq_flush_plug_list(struct blk_plug *plug, bool from_schedule)
{
	...
	struct request *rq;
	LIST_HEAD(list);
	LIST_HEAD(ctx_list);//局部ctx链表
	unsigned int depth;
    //就是令list指向plug->mq_list的吧
	list_splice_init(&plug->mq_list, &list);
    //对plug->mq_list链表上的req进行排序吧，排序规则是req的扇区起始地址
	...
	while (!list_empty(&list)) {
        ...
        //this_ctx是上一个req的软件队列，rq->mq_ctx是当前req的软件队列。二者软件队列相等则if不成立，把req添加到ctx_list链表
        //如果二者软件队列不等，则执行if里边的blk_mq_sched_insert_requests把ctx_list链表上的req插入到硬件队列或者软件队列，
        //同时把ctx_list链表清空。重复上述循环
...
//间接启动req硬件派发。如果硬件队列容纳的req达到上限，硬件队列变忙，则把剩余的plug->mq_list链表的上的req插入到软件队列ctx->rq_list链表上。
			...
			this_ctx = rq->mq_ctx;
			this_q = rq->q;
            //把ctx_list上的req插入硬件和软件队列后，ctx_list链表清0，depth也清0
			depth = 0;
		}
	 ...
	 */
	//感觉这里是，如果plug->mq_list上的req，rq->mq_ctx都指向同一个软件队列，上边的blk_mq_sched_insert_requests执行不了，
	//在这里执行一次，把ctx_list链表上的req插入到硬件队列和软件队列
	if (this_ctx) {
		trace_block_unplug(this_q, depth, from_schedule);
...
static int __blk_mq_issue_directly(struct blk_mq_hw_ctx *hctx, struct request *rq)//req来自当前进程plug->mq_list链表
{
	struct request_queue *q = rq->q;
	struct blk_mq_queue_data bd = {
		.rq = rq,
	...
	switch (ret) {
	case BLK_MQ_RQ_QUEUE_OK:
		blk_mq_update_dispatch_busy(hctx, false);//看着就hctx->dispatch_busy = ewma
		break;
	case BLK_MQ_RQ_QUEUE_BUSY:
	case BLK_MQ_RQ_QUEUE_DEV_BUSY:
		blk_mq_update_dispatch_busy(hctx, true);
...
static int __blk_mq_try_issue_directly(struct blk_mq_hw_ctx *hctx,
						struct request *rq,//req来自当前进程plug->mq_list链表
						bool bypass_insert)
{
	struct request_queue *q = rq->q;
	bool run_queue = true;
    ...
    //从硬件队列hctx有关的blk_mq_tags结构体里得到的req一席之地，有空闲位置可以给req时，
    //接着hctx->tags->rqs[rq->tag]=rq，rq或者req来自当前进程的plug->mq_list链表，现在赋值到了硬件队列hctx->tags->rqs[rq->tag]
    //结构，就建立了req与硬件队列的关系。如果从blk_mq_tags得到空闲的req成功，if成立
	if (!blk_mq_get_driver_tag(rq, NULL, false)) {
		blk_mq_put_dispatch_budget(hctx);//没啥实质操作
		goto insert;
	}
    
    //调用nvme_queue_rq，根据req设置nvme command,把req添加到q->timeout_list，并且启动q->timeout定时器,把新的nvme command
    //复制到nvmeq->sq_cmds[]队列，这看着是req直接发给nvme硬件传输了
...
int blk_mq_request_issue_directly(struct request *rq)//req来自当前进程plug->mq_list链表
{
	int ret;
	int srcu_idx;
    //req所在的软件队列
...
//直接发给nvme硬件传输了。如果不能直接发给nvme硬件传输，则把req添加到硬件队列hctx->dispatch队列，间接启动req硬件派发
		...
		if (ret != BLK_MQ_RQ_QUEUE_OK) {
			if (ret == BLK_MQ_RQ_QUEUE_BUSY ||
				...
				//硬件队列繁忙，则把req添加到硬件队列hctx->dispatch队列，间接启动req硬件派发
				blk_mq_request_bypass_insert(rq,
							list_empty(list));
                //注意，队列繁忙的话，直接直接跳出循环，函数返回了
				break;
			}
...
//这个合并跟软件队列和硬件队列没有半毛钱的关系吧
  ...
  //这个函数看着不复杂呀，就是依次遍历软件队列ctx->rq_list链表上的req，然后看req能否与bio前项或者后项合并
	...
	if (unlikely(is_flush_fua)) {//flush
	...
	} else if (plug && q->nr_hw_queues == 1) {//硬件队列数1
        ...
        //当前进程的plug_list链表上的req数据量大于BLK_MAX_REQUEST_COUNT，强制把req刷到磁盘
		if (request_count >= BLK_MAX_REQUEST_COUNT || (last &&
		    blk_rq_bytes(last) >= BLK_PLUG_FLUSH_SIZE)) {
			blk_flush_plug_list(plug, false);
			trace_block_plug(q);
		}
        //把req添加到plug->mq_list链表上
...
struct blk_mq_tags *blk_mq_alloc_rq_map(struct blk_mq_tag_set *set,
					unsigned int hctx_idx,
					unsigned int nr_tags,//竟然是set->queue_depth，最大的硬件队列数??????????
					unsigned int reserved_tags)
{
	struct blk_mq_tags *tags;
    //分配并返回blk_mq_tags结构，设置其成员nr_reserved_tags和nr_tags
...
//针对hctx_idx编号的硬件队列，每一层队列深度都分配request(共分配depth个request)赋值于tags->static_rqs[]。具体是分配N个page，将page的内存一片片分割成request集合大小
...
/*对 struct blk_mq_hw_ctx *hctx硬件队列结构大部分成员赋值。赋值hctx->tags的blk_mq_tags，每个硬件队列对应一个blk_mq_tags
赋值hctx->ctxs软件队列数组。为硬件队列结构hctx->sched_tags分配blk_mq_tags，这是调度算法的tags，一个硬件队列一个blk_mq_tags，
然后根据为这个blk_mq_tags分配q->nr_requests个request，存于tags->static_rqs[]*/
		...
		struct blk_mq_hw_ctx *hctx, unsigned hctx_idx)
{
    ...
    //为硬件队列结构hctx->sched_tags分配blk_mq_tags，一个硬件队列一个blk_mq_tags，这是调度算法的blk_mq_tags，与硬件队列专属的blk_mq_tags不一样
    //然后根据为这个blk_mq_tags分配q->nr_requests个request，存于tags->static_rqs[]
...
//依次取出每个CPU唯一的软件队列struct blk_mq_ctx *__ctx ，__ctx->cpu记录CPU编号，还根据编号取出该CPU对应的硬件队列blk_mq_hw_ctx
//我感觉没有什么实质的操作!!!!!!
...
{
	int ret = 0;
    //分配返回blk_mq_tags结构，分配设置其成员nr_reserved_tags、nr_tags、rqs、static_rqs
    //hctx_idx是硬件队列编号，每一个对应队列对一个blk_mq_tags结构
    ...
    //针对hctx_idx编号的硬件队列，每一层队列深度都分配request赋值于tags->static_rqs[]。具体是分配N个page，将page的内存一片片分割成request集合大小
    //然后tags->static_rqs记录每一个request首地址，然后执行nvme_init_request()底层驱动初始化函数,建立request与nvme队列的关系吧
						...
						struct request_queue *q)
{
	...
	mutex_lock(&q->sysfs_lock);
    //这个循环看着就是对每个硬件队列结构blk_mq_hw_ctx分配并初始化。每个硬件队列都有唯一的blk_mq_hw_ctx结构，赋值hctx->tags的blk_mq_tags，然后
    //为硬件队列结构hctx->sched_tags分配blk_mq_tags，这是调度算法的tags,然后根据为这个blk_mq_tags分配q->nr_requests个request，存于tags->static_rqs[]
						  ...
						  struct request_queue *q)
{
    ...
    //对每个硬件队列结构blk_mq_hw_ctx分配并初始化。每个硬件队列都有唯一的blk_mq_hw_ctx结构，赋值hctx->tags的blk_mq_tags
    //为硬件队列结构hctx->sched_tags分配blk_mq_tags，这是调度算法的tags,然后根据为这个blk_mq_tags分配q->nr_requests个request，
    //存于tags->static_rqs[]
...
int blk_mq_alloc_tag_set(struct blk_mq_tag_set *set)
{
    ...
    //分配mq_map[]数组，按照CPU的个数分配nr_cpu_ids个unsigned int类型数据，该数组成员对应一个CPU
	set->mq_map = kzalloc_node(sizeof(*set->mq_map) * nr_cpu_ids,
			GFP_KERNEL, set->numa_node);
	if (!set->mq_map)
		goto out_free_tags;
    //为每个set->mq_map[cpu]分配一个硬件队列编号
