...
{
    //inflight不为0，说明队列中有rq要传输，now是当前jiffies
	if (inflight) {
...
void part_round_stats(struct request_queue *q, int cpu, struct hd_struct *part)
{
    ...
    //我一直有个疑问，就是假如磁盘数据传输的很快，几十us就完成了，根本没有达到一个jiffes的时间10ms，那怎么办?
    //那就是这个if (part->stamp != now)起作用了，if不成立，stats为0，就不会
	if (part->stamp != now)
		stats |= 1;
    //part->partno应该代表的是磁盘分区，比如sda2
	if (part->partno) {
        //part2代表的是整个磁盘，比如sda
    ...
    //如果req传输的时间小于一个jiffes时间，直接返回
	if (!stats)
		return;
    
    //part_round_stats->part_in_flight->blk_mq_in_flight
    /*这里的作用，最终在blk_mq_in_flight()令inflight[0]++和inflight[1]++，就是说，957的内核直接执行blk_mq_in_flight()令
     inflight[0/1]++，这样执行下边的part_round_stats_single(),就一定把io_tick累加到diskstat。而957.27内核在blk_mq_in_flight()
     开头加了限制，只有started状态的rq才运行inflight[0/1]++。否则inflight[0/1]就保持0，这样下边的part_round_stats_single
     就不会把io_tick累加到diskstat了，所以才会cat /proc/diskstat看到的io_ticks增加的就慢了，所以才会IO使用率低吧!!!!!
     我只要做一个实验，就能证实我的猜想，就是在part_round_stats_single函数验证，有很多inflight是0*/
	part_in_flight(q, part, inflight);
    //主磁盘分区，比如sda，inflight[1]=主分区struct hd_struct的part->in_flight[0]加上part->in_flight[1]
	if (stats & 2)
		part_round_stats_single(q, cpu, part2, now, inflight[1]);
    //当前块设备的磁盘分区，比如sda2，inflight[0]=当前分区struct hd_struct的part->in_flight[0]加上part->in_flight[1]
	if (stats & 1)
