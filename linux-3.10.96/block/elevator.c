...
static void elv_rqhash_add(struct request_queue *q, struct request *rq)
{
    ...
    //req¸ù¾İÉÈÇø½áÊøµØÖ·rq_hash_key(rq)Ìí¼Óµ½IOµ÷¶ÈËã·¨µÄhashÁ´±íÀï£¬×öhashË÷ÒıÊÇÎªÁËÔÚIOËã·¨¶ÓÁĞÀïËÑË÷¿ÉÒÔºÏ²¢µÄreqÊ±£¬Ìá¸ßËÑË÷ËÙ¶È
...
static struct request *elv_rqhash_find(struct request_queue *q, sector_t offset)//offsetÊÇbio->bi_sector£¬
{
    ...
    //ĞÂµÄreq¿¿Õâ¸öhashÌí¼Óµ½IOµ÷¶ÈËã·¨µÄhashÁ´±íe->hashÀï£¬×öhashË÷ÒıÊÇÎªÁËÔÚIOËã·¨¶ÓÁĞÀïËÑË÷¿ÉÒÔºÏ²¢µÄreqÊ±£¬Ìá¸ßËÑË÷ËÙ¶È
    //ÕâÀï±éÀúhashÁ´±íÉÏµÄreq
	hash_for_each_possible_safe(e->hash, rq, next, hash, offset) {
        ...
        //reqµÄÉÈÇø½áÊøµØÖ·µÈÓÚoffset£¬offsetÊÇbioµÄÉÈÇøÆğÊ¼µØÖ·(¸Ãº¯Êı´«²ÎÊÇoffset=blk_rq_pos(rq))£¬ÕâÑùbio¾ÍÊÇºóÏîºÏ²¢µ½req
        //hash keyÊÇreqµÄÉÈÇø½áÊøµØÖ·
 ...
 * in a sorted RB tree.
 */
//°´ÕÕreqµÄ´ÅÅÌÆğÊ¼µØÖ·°ÑreqÌí¼Óµ½ºìºÚÊ÷¶ÓÁĞÀï£¬Õâ¸öºìºÚÊ÷ÀïreqµÄÅÅÁĞ¹æÔòÊÇ£¬Ë­µÄ´ÅÅÌÆğÊ¼µØÖ·Ğ¡Ë­¿¿×ó
void elv_rb_add(struct rb_root *root, struct request *rq)
{
	...
	while (*p) {
        ...
        //rqµÄ´ÅÅÌÆğÊ¼µØÖ·±È__rqµÄĞ¡£¬ÄÇrqÓ¦¸Ã²åÈëµ½×ó×ÓÊ÷£¬ÏÔÈ»£¬Ë­µÄ´ÅÅÌÆğÊ¼µØÖ·Ğ¡£¬Ë­¿¿×ó
		if (blk_rq_pos(rq) < blk_rq_pos(__rq))
			p = &(*p)->rb_left;
        //·ñÔò£¬rqµÄ´ÅÅÌÆğÊ¼µØÖ·>=__rqµÄ£¬ÔòrqÓ¦¸Ã²åÈëµ½ÓÒ×ÓÊ÷
...
struct request *elv_rb_find(struct rb_root *root, sector_t sector)
{
	...
	while (n) {
        //´Óroot¸ù½Úµã¿ªÊ¼±éÀú£¬ÒÀ´ÎÈ¡³öºìºÚÊ÷ÖĞµÄÒ»¸öreq
    ...
    //Èç¹û×ßµ½ÕâÀï£¬Ó¦¸ÃÊÇºìºÚÊ÷¶ÓÁĞ¾ÍÃ»ÓĞreq³ÉÔ±°É£¬ÊÇ¿ÕµÄ£¬ÒòÎªÖ»ÒªºìºÚÊ÷ÓĞÒ»¸öreq,ÄÇbioµÄ´ÅÅÌ½áÊøµØÖ·£¬¿Ï¶¨ ´óÓÚ»òĞ¡ÓÚ»òµÈÓÚ reqµÄÆğÊ¼µØÖ·°É
    //´íÁË´íÁË£¬Ö»ÓĞsector = blk_rq_pos(rq)²Å»áreturn rq£¬Èç¹ûÃ»ÓĞÕÒµ½Æ¥ÅäµÄreq,n = n->rb_right/rb_left±éÀúµ½Ê÷µ×£¬nÎªNULLÍË³öÑ­»·
	return NULL;
}
 ...
 * specific elevators.
 */
//°ÑreqÌí¼Óµ½rqµÄqueue_head¶ÓÁĞ£¬½«À´´ÅÅÌÇı¶¯³ÌĞò¾ÍÊÇ´Óqueue_headÁ´±íÈ¡³öreq´«ÊäµÄ
...
/*    -----Ô­Ê¼ÄÚºËµÄ£¬ÏÂ±ßµÄÊÇ3.10.0.957.27ÄÚºË£¬¾ÍÔö¼ÓÁËe->aux->ops.mq.request_merge
...
void elv_merged_request(struct request_queue *q, struct request *rq, int type)
{
	...
	/*Ã²ËÆdeadlineµ÷¶ÈËã·¨µÄºìºÚÊ÷£¬ÔÚ²åÈëreqÊ±£¬¾ÍÊÇ°´ÕÕreq´ú±íµÄÉÈÇøÆğÊ¼µØÖ·À´¶Ô±È£¬Ë­µÄÉÈÇøÆğÊ¼µØÖ·Ğ¡£¬Ë­ÅÅÁĞ¿¿×ó.
	  ¼û deadline_add_request->deadline_add_rq_rb->elv_rb_add²åÈëreqºÍelv_merge->deadline_merge->elv_rb_find±éÀúreq¡£
	  blk_mq_sched_try_merge->elv_merged_request->deadline_merged_requestÖØĞÂreqÅÅĞò£¬Õë¶Ôdeadlineµ÷¶ÈËã·¨µÄºìºÚÊ÷¶ÓÁĞ£¬
...
//ĞÂ·ÖÅäµÄreq²åÈëIOËã·¨¶ÓÁĞ£¬»òÕßÊÇ°Ñµ±Ç°½ø³ÌplugÁ´±íÉÏreqÈ«²¿²åÈëµ½IOµ÷¶ÈËã·¨¶ÓÁĞ
void __elv_add_request(struct request_queue *q, struct request *rq, int where)//whereÄ¬ÈÏÊÇELEVATOR_INSERT_SORT
{
	...
	case ELEVATOR_INSERT_BACK://ºóÏòºÏ²¢
		 ...
		 *   processing.
		 */
		//ÕâÀïµ÷ÓÃµ×²ãÇı¶¯Êı¾İ´«Êäº¯Êı£¬¾Í»á´ÓrqµÄqueue_head¶ÓÁĞÈ¡³öreq·¢ËÍ¸ø´ÅÅÌÇı¶¯È¥´«Êä
		__blk_run_queue(q);
		break;

	case ELEVATOR_INSERT_SORT_MERGE://°Ñ½ø³Ì¶ÀÓĞµÄplugÁ´±íÉÏµÄreq²åÈëIOµ÷¶ÈËã·¨¶ÓÁĞÀï×ßÕâÀï
		/*
	...
	case ELEVATOR_INSERT_SORT://ĞÂ·ÖÅäµÄreq²åÈëµÄIOµ÷¶ÈËã·¨¶ÓÁĞ×ßÕâÀï
		...
		q->nr_sorted++;
		if (rq_mergeable(rq)) {
            //ĞÂµÄreq¿¿rq->hashÌí¼Óµ½IOµ÷¶ÈËã·¨µÄhashÁ´±íÀï
			elv_rqhash_add(q, rq);
			if (!q->last_merge)
				q->last_merge = rq;
		}
...
//Ö»ÊÇ´ÓIOµ÷¶ÈËã·¨¶ÓÁĞÀïÈ¡³örqµÄÏÂÒ»¸örq°É,²»Í¬µÄµ÷¶ÈËã·¨µ÷¶È¶ÓÁĞ²»Ò»Ñù
struct request *elv_latter_request(struct request_queue *q, struct request *rq)
{
	struct elevator_queue *e = q->elevator;

	if (e->type->ops.elevator_latter_req_fn)//elv_rb_latter_requestºÍnoop_latter_request
...
{
	struct elevator_queue *e = q->elevator;
    //elv_rb_former_requestºÍnoop_former_request
...
//Ó¦¸ÃÊÇÕÒµ½IOµ÷¶ÈËã·¨¶ÓÁĞÀïµÄrqµÄÏÂÒ»¸örq£¬Õâ¸ö¶ÓÁĞÃ²ËÆÖ»ÊÇ¿¿×Årq->rb_node¹¹³ÉµÄÒ»¸öÊ÷ĞÎ¶ÓÁĞÑ½
