...
static void drive_stat_acct(struct request *rq, int new_io)//高版本内核函数是 blk_account_io_start
{
	...
	if (!new_io) {//说明这个新的bio合并到了rq
		part = rq->part;
		part_stat_inc(cpu, part, merges[rw]);//增加合并计数
	} else {
...
retry:
    //分配rq，这里设置req->start_time为jiffies
	...
	goto retry;//不是在这里就return了，而是再跳到上方
}
...
/*这里有个问题必须要考虑，blk_account_io_start/blk_account_io_done/drive_stat_acct->part_round_stats->part_round_stats_single,
首先要now != part->stamp，至少每part_round_stats_single函数被执行的时间要大于一个jiffies，才能增加io_ticks计数，本身也是以jiffies为单位。
同时还要part_in_flight(part)不为0，才会执行增加io_ticks计数。part_in_flight(part)不为0，就是part->in_flight[0/1]不为0，
这表示该磁盘分区有req要传输。每当执行blk_account_io_start或者drive_stat_acct启动req传输时，执行
part_inc_in_flight() 令part->in_flight[0/1]++。当req传输完成后，执行blk_account_io_done->part_dec_in_flight令part->in_flight[0/1]--。*/
				    ...
				    unsigned long now)
{
    ...
    //in_flight 不为0，说明in flight队列有req
	if (part_in_flight(part)) {
        //time_in_queue显然与IO队列中的总IO数在IO队列中的时间有关
 ...
 */
//更新主块设备和块设备分区的time_in_queue和io_ticks数据，blk_account_io_done() req传输完成执行
void part_round_stats(int cpu, struct hd_struct *part)
{
	unsigned long now = jiffies;

	if (part->partno)//更新主块设备的io_ticks，并更新part->stamp 为当前时间
		part_round_stats_single(cpu, &part_to_disk(part)->part0, now);
    
    //更新当前设备分区的io_ticks，并更新part->stamp 为当前时间
...
/*bio合并到req必然要调用bio_attempt_back_merge/bio_attempt_front_merge函数，不管是bio合并到plug链表的req，还是
bio合并到IO算法队列的req*/
			       ...
			       unsigned int *request_count)
{
	...
	if (!plug)
		goto out;
	*request_count = 0;
    //依次取出进程plug->list链表上已有的struct request req，然后判断rq代表的磁盘范围是否挨着本次的bio的范围，是则把bio合并到req
	list_for_each_entry_reverse(rq, &plug->list, queuelist) {
		int el_ret;
        //比较req队列是否同一个，这种情况应该很容易成立吧，只要是取出的req所在的块设备与q代表的是同一个块设备，很容易成立
		if (rq->q == q)
			(*request_count)++;
         //只有二者属于同一个req队列，并且通过合并检查才能执行下边的合并
...
void blk_queue_bio(struct request_queue *q, struct bio *bio)
{
    ...
    /*如果没有设置IO调度算法，下边的elv_merge()执行后就是直接返回ELEVATOR_NO_MERGE吧????????????????*/
	...
	el_ret = elv_merge(q, &req, bio);/*req变量 保存bio可以合并的req指针*/
	if (el_ret == ELEVATOR_BACK_MERGE) {
	    //后项合并，bio合并到req对应的磁盘空间范围后边
		if (bio_attempt_back_merge(q, req, bio)) {
            //调用IO调度算法的elevator_bio_merged_fn函数，貌似只有增加一部分统计数据
...
get_rq:
	 ...
	 */
	//得到一个新的rq，里边有可能休眠,这里设置req->start_time为jiffies
	req = get_request(q, rw_flags, bio, GFP_NOIO);//从get_request返回还是上着q->queue_lock锁
	if (unlikely(!req)) {
...
void generic_make_request(struct bio *bio)
{
	...
	if (!generic_make_request_checks(bio))//blkio限流检查
		return;
...
/*
 ...
 3 先遍历req上的每一个bio，再得到每个bio的bio_vec，把bio对应的文件数据在内存中的首地址bvec->bv_pag+bvec->bv_offset写入scatterlist
   scatterlist是磁盘数据DMA传输有关的数据结构，scatterlist保存到bidi_sdb->table.sgl，bidi_sdb是req的struct scsi_data_buffer成员。
 */
struct request *blk_peek_request(struct request_queue *q)
{
    ...
    //如果q->queue_head没有req，则执行deadline_dispatch_requests从fifo队列选择派发的req到q->queue_head链表
    ...
    /*
     1 分配一个struct scsi_cmnd *cmd，使用req对cmd进行部分初始化cmd->request=req，req->special = cmd，还有cmd->transfersize传输字节数
         、cmd->sc_data_direction DMA传输方向
     2 先遍历req上的每一个bio，再得到每个bio的bio_vec，把bio对应的文件数据在内存中的首地址bvec->bv_pag+bvec->bv_offset写入scatterlist
       scatterlist是磁盘数据DMA传输有关的数据结构，scatterlist保存到bidi_sdb->table.sgl，bidi_sdb是req的struct scsi_data_buffer成员。
     */
...
/* 1 执行blk_account_io_completion增加sectors IO使用计数，即传输的扇区数。更新req->__data_len和req->buffer
   2 依次取出req->bio链表上所有req对应的bio,一个一个更新bio结构体成员数据，执行bio的回调函数，唤醒休眠的进程*/
bool blk_update_request(struct request *req, int error, unsigned int nr_bytes)
{// nr_bytes在scsi_finish_command()中赋值。是本次磁盘硬件实际传输完成的数据量，并不一定是req所有的磁盘数据量。所以
 //req对应的的磁盘数据不能保证一次就全部传输完成呀，我去，这有点骗人，本来以为req的合并了多个bio，可以保证一次就把所有IO数据传输完
	int total_bytes;
    ...
    //这个循环会依次取出req->bio链表上所有req对应的bio,一个一个更新bio结构体成员数据，执行bio的回调函数，唤醒休眠的进程
    /*nr_bytes代表本次实际传输完成的磁盘数据量，并不一定是req对应的全部磁盘数据，所以该循环if (!nr_bytes)很容易成立，只要把
      实际传输完成磁盘数据的几个bio遍历完，nr_bytes就是0了，就会跳出循环。*/
	while (req->bio) {
		struct bio *bio = req->bio;
		unsigned bio_bytes = min(bio->bi_size, nr_bytes);

        //很容易成立，bio_bytes大概率就是bio->bi_size，然后令req->bio指向req的下一个bio。之后有两种情况，如果本次实际传输完的
        //字节数bio_bytes就是bio->bi_size，则下边nr_bytes为0，跳出循环。req->bio = bio->bi_next指向的bio，紧接着就会传输。如果
        //本次实际传输完的字节数bio_bytes大于bio->bi_size，说明本次有多个bio参与磁盘数据传输，那就执行req_bio_endio唤醒等待bio传输的
        //进程。循环，直到把本次参与传输的bio全部遍历完。如果req的bio都参与传输了，则req->bio = bio->bi_next=NULL。
...
{
    //如果req上的bio全部传输完成，返回false。否则还有bio要传输，返回true
...
static void blk_finish_request(struct request *req, int error)
{
	...
	if (req->end_io)
		req->end_io(req, error);//回调函数，唤醒等待IO传输完成而休眠的进程
	else {
		if (blk_bidi_rq(req))
...
/*取出当前进程plug->mq_list链表上的req，如果有IO调度算法，则把req插入elv的fifo队列，
mq-deadline算法的还要插入红黑树。如果没有IO调度算法，在硬件队列空闲时，尝试把plug->mq_list链表的上的req建立与
...
//这个函数 blk_finish_plug->blk_flush_plug_list->queue_unplugged->__blk_run_queue->__blk_run_queue_uncond->mmc_request_fn 发送emmc命令给emmc控制器
