...
#endif
/* 一个调度实体(红黑树的一个结点)，其包含一组或一个指定的进程，包含一个自己的运行队列，
一个父亲指针，一个指向需要调度的运行队列指针 */
//运行队列cfs_rq用红黑树组织调度实体，cfs运行队列和红黑树队列一回事
struct sched_entity {
    //调度实体的权重，决定了虚拟运行时间，决定实体在cfs红黑树的位置
	struct load_weight	load;		/* for load-balancing */
    //调度实体以run_node插进运行队列红黑树
	...
	//虚拟运行时间，权重越大，增长速度越慢，越处于cfs红黑树队列左边，越容易被调度
	u64			vruntime;
...
struct task_struct {
	...
	struct sched_entity se;//对应的cfs调度实体
	struct sched_rt_entity rt;//
#ifdef CONFIG_CGROUP_SCHED
	struct task_group *sched_task_group;//所在进程组
#endif
...
static inline void setup_thread_stack(struct task_struct *p, struct task_struct *org)
{
	...
	*task_thread_info(p) = *task_thread_info(org);
    //p->stack->task = p 指向本进程task_struct结构
